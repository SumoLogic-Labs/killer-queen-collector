...
class EventJsonSerializerTest extends TestBase {
  ...
  "EventJsonSerializer" should {
    "convert objects to get json" in {
      ...
      val estimatedTime = (System.currentTimeMillis() - gs.startTime) / 1000
      val event = ConnectedEvent(1)
      val sut = new EventJsonSerializer(gs)
      sut.toJson(event) should be(
        s"""{"connectionId":1,"event":"${event.event}","rawValue":"${event.rawValue}","gameId":${gs.id},"apiVersion":${sut.ApiVersion},"secondsSinceGameStart":$estimatedTime}""")
    }
  }
